package utils.loka.dataBase.scroll  {
	...
	public class Btndown extends ScrollBase{
		//private var _btnEffect:BtnEffect;
		public function Btndown(data:Array) {
		...
		public function create(w:Number = 10, h:Number = 10, x:Number = 0, y:Number = 90):void {
			if (this.issetData("btnDown")) {
				this.addChild(BitmapEdit.drawImg(this.getDataByName("btnDown").data));
			}else{
				this.graphics.beginFill(this._bckgColor,0.1);
					...
					this.graphics.beginFill(this._bckgColor,0.2);
						this.graphics.moveTo(w / 2, h-h / 10);
						this.graphics.lineTo(w - w / 10, h / 10);
						this.graphics.lineTo(w / 10, h / 10);
						this.graphics.lineTo(w / 2, h - h / 10);
		...
		private function clicked(e:Event):void {
			//this.startDrag();
			dispatchEvent(new Event(MyscrollEvent.BTN_DOWN_CLICKED,true));
		}
	}
